...
CirMgr::printFloatGates()
{
   ...
   if(_gateIDswithFloat.size() != 0){
      cout << "Gates with floating fanin(s):";
      for(it = _gateIDswithFloat.begin();it != _gateIDswithFloat.end();++it) cout << " " << (*it);
      cout << endl;
   }
   int count = 0;
   for(iter = _gateList.begin();iter != _gateList.end();++iter){
      if(iter -> second -> _type == PO_GATE) continue;
      else if(iter -> second -> _type == CONST_GATE) continue;
      else if(iter -> second -> _type == UNDEF_GATE) continue;
      else{
         if(iter -> second -> _fanoutList.size() == 0) count++;
      }
   }
   if(count != 0){
      ...
      for(iter = _gateList.begin();iter != _gateList.end();++iter){
         if(iter -> second -> _type == PO_GATE) continue;
            ...
            if(iter -> second -> _fanoutList.size() == 0) cout << " " << iter -> first;
         }
      }
      cout << endl;
   }
...
CirMgr::writeAag(ostream& outfile)
{
   ...
   for(int i = 0;i < _dfsList.size();++i){
      if(_dfsList.at(i) -> _type == AIG_GATE) A++;
   }
   outfile << "aag";
...
CirMgr::readInput(ifstream& fi){
   ...
   //    }
   //    if(inLtr % 2 != 0){
   //       cerr << "input literal ID must be even number." << endl;
   //       return false;
   //    }
   //    gateID = inLtr / 2;
   ...
   for(int i = 0;i < miloa[1];++i){
      ...
      for(int j = 0;j < i;++j){
         if(inLtr == allinLtr.at(j)){
            errGate = _piList.at(j);
            errInt = inLtr;
...
CirMgr::readOutput(ifstream& fi){
   // unsigned gateID;
   // unsigned outLtr;
   // unsigned num = miloa[3];
   // for(int i = 0;i < num;i++){
   //    if(!(fi >> outLtr)){
   //       cerr << "can't find output or it's not a non-negative integer" << endl;
   //       return false;
   //    }
   //    gateID = miloa[0] + 1 + i;
...
CirMgr::readAig(ifstream& fi){
   ...
   //    for(int j = 0;j < 3;j++){
   //       if(!(fi >> content[j])){
   //          cerr << "can't find AIG or it's not a non-negative integer" << endl;
   //          return false;
   //       }
   //    }
   //    if(content[0] % 2 != 0){
   //       cerr << "LHS must be even number." << endl;
   //       return false;
   //    }
   //    gateID = content[0] / 2;
      ...
      for(int j = 0;j < 3;++j){
         ...
         if(j == 0){
            ...
            for(it = _gateList.begin();it != _gateList.end();++it){
               if(content[0] == it -> first * 2){
                  errGate = it -> second;
                  errInt = content[0];
                  return parseError(REDEF_GATE);
               }
            }
            // it = _gateList.insert(content[0] / 2);
            // if(it -> second == true){
            //    errGate = _gateList.at(content[0] / 2);
            //    errInt = content[0];
            //    return parseError(REDEF_GATE);
            // }
         }
...
CirMgr::readSymbol(ifstream& fi){
   ...
   // // delete ignore after read func change to getline? 
   // fi.ignore();
   ...
   //       cerr << "wrong char (should be i or o)" << endl;
   //       return false;
   //    }
   //    symbols.push_back(line);
   //    ////cout << "into symbol" << line << endl;
   //    ++lineNo;
   // }
   ...
   //    }
   //    unsigned id;
   //    stringstream ss((*it).substr(1));
   //    if(ss >> id){
   //       //todo cannot use size because ex id = 100
   //       if((*it).size() >= 3){
   //          string symbName = (*it).substr(3);
   //          // index in order? error detect here
   //          ////cout << id << endl;
   //          if((*it).at(0) == 'i' && id < miloa[1]) _piList.at(id) -> _symbol = symbName;
   //          else if((*it).at(0) == 'o' && id < miloa[3]) _poList.at(id) -> _symbol = symbName;
   //          else{
   //             cerr << "too big" << endl;
   //             return false;
   //          }
   ...
   //       }
   //       else{
   //          cerr << "missing symbolic name" << endl;
   //          return false;
   //          // todo detect whitspace
   //       }
   //    }
   //    else{
   //       cerr << "not unsigned int" << endl;
   //       return false;
   //    }
   // }
   // return true;
