...
#define toSizeT(t)      SIZE_T * (t / SIZE_T + 1) - (!(t % SIZE_T) ) * SIZE_T // TODO
//
...
private:
   ...
   //        the _recycleList[]
   size_t getArraySize(size_t t) const {    //including 8 bytes for array size because it is before new() 
      assert(t % SIZE_T == 0);
      assert(t >= S);
      // TODO
      return (t - SIZE_T) / S;
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      while(p -> _arrSize != n){
         if(p -> _nextList == NULL){
            MemRecycleList<T>* _new = new MemRecycleList<T>(n);
            p -> _nextList = _new;
            return p -> _nextList;
         }
         p = p -> _nextList;
   ...
   T* getMem(size_t t) {
      ...
      // TODO
         ...
         if(!_activeBlock -> getMem(t, ret)){
            size_t rn, remainSize;
            remainSize = _activeBlock -> getRemainSize();
            if(remainSize >= S){
               rn = getArraySize(remainSize);
   ...
   size_t getNumBlocks() const {
      // TODO
      MemBlock<T>* _ptr = _activeBlock;
      if(_ptr == NULL) return 0;
      else{
         size_t num = 1;
         while(_ptr -> _nextBlock != NULL){
