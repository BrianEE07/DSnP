...
CmdParser::openDofile(const string& dof)
{
   ...
   if(_dofileStack.size() < max_recursion_depth)
...
CmdParser::printHelps() const
{
   ...
   for(it = _cmdMap.begin();it != _cmdMap.end();it++) it -> second -> help();
   cout << endl;
}
...
CmdParser::parseCmd(string& option)
{
   ...
   string cmd;
   size_t idx_end;
   idx_end = myStrGetTok(str, cmd);
   CmdExec* e = getCmd(cmd);
   if(str.size() > cmd.size()) option = str.substr(idx_end, str.size() - cmd.size());
   if(e == 0) cerr << "Illegal command!! (" << cmd << ")";   //getCmd returns 0 when illegal command
   else return e;
...
CmdParser::listCmd(const string& str)
{
   ...
   // 1. LIST ALL COMMANDS
   if(idx == string::npos){ // null or only spaces
      CmdMap::const_iterator it;
      cout << endl;
      int count = 0;
   ...
   // 3. LIST THE SINGLY MATCHED COMMAND
   // 4. NO MATCH IN FITST WORD
   else if(idx != string::npos && str.find_first_of(" ", idx) == string::npos){ // spaces only before first letters
      string tok = str.substr(idx, str.size() - idx);
      CmdMap::const_iterator it;
      CmdMap::const_iterator sin_mat_it; //single match's it
      int count = 0;
      bool none_match = false;
      bool single_match = false;
      for(it = _cmdMap.begin();it != _cmdMap.end();it++)
         if(myStrNCmp(it -> first + it -> second -> getOptCmd(), tok, 1) == 0){
            count++;
            sin_mat_it = it;
         }
      if(count == 0) none_match = true;
      if(count == 1) single_match = true;
      count = 0;
      if(none_match) mybeep();
      else if(single_match){
         string cmd = sin_mat_it -> first + sin_mat_it -> second -> getOptCmd();
         if(cmd.size() > tok.size()){
            string need_to_print = cmd.substr(tok.end() - tok.begin(), cmd.size() - tok.size());
            char n_t_p[100];
            strcpy(n_t_p, need_to_print.c_str());
            for(unsigned long int i = 0;i < need_to_print.size();i++)
               insertChar(n_t_p[i]);
            insertChar(' ');
         }
         else insertChar(' ');
         ...
         if(myStrNCmp(it -> first + it -> second -> getOptCmd(), tok, 1) == 0){
            ...
            none_match = false;
            if(count % 6 == 0 && count != 0) cout << endl;   // not good when matches = 11 but it won't
            cout << setw(12) << left << cmd;
            }
         }
         reprintCmd();
   ...
   // 5. FIRST WORD ALREADY MATCHED ON FIRST TAB PRESSING
   // 6. FIRST WORD ALREADY MATCHED ON SECOND AND LATER TAB PRESSING
   else if(idx != string::npos && str.find_first_of(" ", idx) != string::npos){  // string before the cursor has already matched a command, and at least one spaces before the cursor
      string token;
      size_t tok_end = myStrGetTok(str, token);
      CmdExec* e = getCmd(token);
      if(e && _tabPressCount == 1){
         cout << endl;
         e -> usage(cout);
         reprintCmd();
      }
         ...
         if(str.back() == ' '){
            ...
            if(files.size() == 0) mybeep();
               ...
               if(_tabPressCount == 2){
                  char fileN[100];
                  strcpy(fileN, files.at(0).c_str());
                  for(unsigned long int i = 0;i < files.at(0).size();i++)
                     insertChar(fileN[i]);
                  insertChar(' ');
               }
               else mybeep();
               ...
               for(int j = 0;;j++){
                  ...
                  for(i = 0;i < files.size() - 1;i++){
                     if(files.at(i).size() > j){
                        if(files.at(i)[j] == files.at(i + 1)[j]){
                          all_same *= 1;
                        }
                        else all_same = false;
                     }
            ...
            bool same = false;
            string ori_prefix;
            string ori_prefix_nospa;
            ori_prefix.append(str.substr(tok_end));
            myStrGetTok(ori_prefix, ori_prefix_nospa);
            listDir(files, ori_prefix_nospa, ".");
            if(files.size() == 0) mybeep();
            else if(files.size() == 1){
               char fileN[100];
               strcpy(fileN, files.at(0).c_str());
               for(unsigned long int i = ori_prefix_nospa.size();i < files.at(0).size();i++)
                  insertChar(fileN[i]);
               insertChar(' ');
            }
            else{
               unsigned long int i;
               int insert_num = 0;
               for(int j = ori_prefix_nospa.size();;j++){
                  same = true;
                  for(i = 0;i < files.size() - 1;i++){
                     if(files.at(i).size() > j){
                        if(files.at(i)[j] == files.at(i + 1)[j]){
