...
DBAppendCmd::exec(const string& option)
{
   ...
   if(options.empty())
      ...
      return CmdExec::errorOption(CMD_OPT_MISSING, options.at(options.size() - 1));
   else if(options.size() > 2){
      for(unsigned long int i = 2;i < options.size();i++){
         extra_options.append(" ");
         extra_options.append(options.at(i));
      }
      return CmdExec::errorOption(CMD_OPT_EXTRA, extra_options);
   }
   else{
      int val;
      if(!myStr2Int(options.at(1), val)) return CmdExec::errorOption(CMD_OPT_ILLEGAL, options.at(1));
      if(!isValidVarName(options.at(0))) return CmdExec::errorOption(CMD_OPT_ILLEGAL, options.at(0));
      DBJsonElem elm(options.at(0), val);
      if(!dbjson.add(elm)){
...
{  
   ...
   if(!CmdExec::lexSingleOption(option, token, 1))   // optional == true
      return CMD_EXEC_ERROR;
   if(token.size()) {
      for(long unsigned int i = 0;i < dbjson.size();i++)
         if(token == dbjson[i].key()){
            cout << "{ " << dbjson[i] << " }";
            return CMD_EXEC_DONE;
         }
      cerr << "Error: No JSON element with key \"" << token << "\" is found.";
      return CMD_EXEC_ERROR;
   }
   else cout << dbjson << "Total JSON elements: " << dbjson.size();
   return CMD_EXEC_DONE;
}
